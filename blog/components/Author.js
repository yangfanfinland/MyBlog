...
const Author = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await axios(`${servicePath.getUserInfo}/fan`).then(
                (res) => {
                    if (res.data.errno === 0) {
            ...
            <div className="author-introduction">
                ...
                <Divider>Social accounts</Divider>
                {
                    socialAccountList.map((account) => (
                        <Avatar key={account.name} style={{cursor:'pointer'}} onClick={() => handleSocialAccountClick(account.url)} size={28} icon={account.icon === "GithubOutlined"? <GithubOutlined />: <LinkedinOutlined /> } className="account" />
                    ))
                }
